...
# Detect PostgreSQL observability capabilities and optionally enable instrumentation.
# This script is careful to:
# - capture before/after settings,
# - gate features by version/capability,
# - avoid touching Uyuni application schema.
...
usage() {
  cat <<USAGE
Usage: bash benchmark-harness/scripts/detect_pg_capabilities.sh [--session-dir <path>]
...
SESSION_DIR=""
# Supports optional explicit session dir so orchestration can keep all artifacts in one tree.
while [[ $# -gt 0 ]]; do
  case "$1" in
    --session-dir)
      SESSION_DIR="$2"
...
if [[ -z "$SESSION_DIR" ]]; then
  SESSION_DIR="$(benchmark_new_session_dir "$BENCHMARK_ROOT/results" "pg-capabilities" "$STORAGE_CLASS")"
fi

# Output files:
# - detect log for audit trail
# - capability JSON for downstream summary
# - before/after settings TSV for human inspection
...
scalar() {
  # Utility for single-value SQL results; strips whitespace for robust shell comparisons.
  local q="$1"
  run_psql_tsv postgres "$q" | tr -d '[:space:]'
}

snapshot_settings() {
  # Record relevant toggles that affect instrumentation and query introspection.
  local out="$1"
...
restart_db_deployment() {
  # Some ALTER SYSTEM changes (e.g., shared_preload_libraries) require restart.
  # We perform a rollout restart and wait for readiness to reduce race conditions.
  local db_deploy
...

{
  ...
  # Baseline snapshot before any potential ALTER SYSTEM changes.
  snapshot_settings "$BEFORE_SETTINGS"

  # Core capability probes:
  # - catalog view presence
  # - extension presence
  # - runtime setting availability
  ...
  # Optional enablement path for pg_stat_statements:
  # - set preload + compute_query_id
  # - restart DB deployment
  # - create extension in benchmark DB
  ...
  # Settings above only need reload (not restart) for track_* toggles.
  ...
  # Final machine-readable capability payload consumed by summarize_benchmark.py.
  jq -n \
