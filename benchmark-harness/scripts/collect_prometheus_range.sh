...
# Query Prometheus `query_range` for configured metrics over one workload run window.
# Saves raw per-query JSON plus compact summary JSON/CSV.
...
usage() {
  cat <<USAGE
Usage: bash benchmark-harness/scripts/collect_prometheus_range.sh \
...
# Strict parsing: all run window identifiers must be present.
while [[ $# -gt 0 ]]; do
  case "$1" in
    --start-epoch)
      START_EPOCH="$2"
...
# Raw outputs are stored per workload/run for reproducibility.
...
# If in-cluster URL is not reachable from host shell, fallback to port-forward.
...
python3 - "$BENCHMARK_ROOT/config/prom_queries.yaml" "$effective_prom_url" "$START_EPOCH" "$END_EPOCH" "$STEP" "$WORKLOAD" "$RUN_ID" "$SESSION_DIR/prometheus/raw" "$SUMMARY_JSON" "$SUMMARY_CSV" <<'PY'
...
def parse_queries_yaml(path: Path) -> list[dict[str, Any]]:
    # Prefer YAML parser when available, fallback to lightweight parser for portability.
...
def percentile(values: list[float], p: float) -> float:
    # Linear interpolation percentile implementation for p95 metric.
    if not values:
        return math.nan
    if len(values) == 1:
        return values[0]
...
for q in queries:
    ...
    try:
        # Store full API payload for traceability, even when summarized values are enough.
        ...
        # Missing series/errors are non-fatal for a run; they are flagged in summary output.
        missing = True
        error = str(exc)
...
with summary_csv_path.open("a", encoding="utf-8", newline="") as fh:
    # Append one row per query so multiple run windows accumulate in one session CSV.
