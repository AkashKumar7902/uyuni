...
#
# Design notes:
# - Keep all benchmark scripts consistent in logging, env loading, and Kubernetes access.
# - Centralize pod detection/SQL execution so each caller does not duplicate fragile logic.
# - Provide fallback pgbench execution through a temporary client pod when DB image lacks pgbench.
...
benchmark_load_env() {
  # Load environment variables from config file when available, then set safe defaults.
  # `set -a` exports sourced variables automatically so all child processes inherit them.
  local env_file
...
benchmark_new_session_dir() {
  # Session naming convention:
  # session-<UTC timestamp>-<profile>-<storage class>
  # This ensures deterministic and sortable evidence directories.
  local outdir="${1:-$BENCHMARK_ROOT/results}"
...
benchmark_save_metadata_snapshot() {
  # Capture baseline context up front so every run can be interpreted later:
  # node list, storage class details, namespace workloads, and full environment.
  local session_dir="$1"
...
get_db_pod() {
  # Uyuni DB pod names usually start with db-; pick the first matching pod.
  local pod
...
get_uyuni_pod() {
  # Prefer runtime Uyuni server pod and skip transient setup pods.
  local pod
...
get_micro_client_pod() {
  # The benchmark workflow assumes a pre-registered micro-client pod in uyuni namespace.
  local pod
...
run_psql_cmd() {
  ...
  # Execute one SQL command with ON_ERROR_STOP so failures propagate reliably.
  local db_name="$1"
  local sql="$2"
  local db_pod
  db_pod="$(get_db_pod)"
...
run_psql_file() {
  ...
  # Stream local SQL file to psql running inside the DB pod.
  local db_name="$1"
  local sql_file="$2"
  local db_pod
  db_pod="$(get_db_pod)"
...
run_psql_tsv() {
  ...
  # Emit plain tab-separated output for easy shell parsing (no headers, unaligned mode).
  local db_name="$1"
  local sql="$2"
  local db_pod
  db_pod="$(get_db_pod)"
...
run_pgbench_cmd() {
  ...
  # Try local DB pod first for speed; fallback to sidecar client pod if pgbench is missing.
  local db_name="$1"
  shift
  local db_pod
  db_pod="$(get_db_pod)"
...
check_pgbench_available() {
  # Fast capability check used by callers before running matrix workloads.
  # If DB pod lacks pgbench, validate that the fallback PG client image has it.
  local db_pod
  db_pod="$(get_db_pod)"
  if kubectl -n "$UYUNI_NAMESPACE" exec "$db_pod" -- sh -lc 'command -v pgbench >/dev/null 2>&1' >/dev/null 2>&1; then
    return 0
  ...
  local pod="benchmark-pgbench-check-$(date -u +%H%M%S)"
...
resolve_db_admin_pass_if_empty() {
  # Best-effort password discovery from common env var names inside the DB pod.
  # This avoids hard failing when DB_ADMIN_PASS is intentionally left blank in env file.
  if [[ -n "$DB_ADMIN_PASS" ]]; then
    return 0
  fi
...
run_pgbench_via_pgclient() {
  # Run pgbench from an ephemeral pod in BENCH_NAMESPACE.
  # This path is used when DB image does not include pgbench binary.
  local db_name="$1"
  shift

  resolve_db_admin_pass_if_empty || die "DB_ADMIN_PASS is required for pgbench fallback via PGCLIENT_IMAGE"

  local pod="benchmark-pgbench-$(date -u +%H%M%S)-$RANDOM"
  local -a pb_args=()
  local idx=1
  local arg_count="$#"

  # Create an idle pod first so custom pgbench scripts can be copied inside.
  # We keep it alive briefly (`sleep`) and exec pgbench after files are staged.
  kubectl -n "$BENCH_NAMESPACE" run "$pod" \
    --image="$PGCLIENT_IMAGE" \
    --restart=Never \
  ...
  # Supports both "-f /path/file.sql" and "-f/path/file.sql" forms.
  ...
  # Build a shell-escaped arg string to preserve flags/values exactly.
  local args_quoted
