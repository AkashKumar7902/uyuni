...
# Prepare dedicated pgbench database used by benchmark runs.
# This script intentionally never modifies the `susemanager` schema/database.
...
usage() {
  cat <<USAGE
Usage: bash benchmark-harness/scripts/prepare_pgbench_db.sh --scale <n> [--recreate] [--session-dir <path>]
...
# CLI parsing kept strict; unknown flags are rejected to avoid accidental mis-runs.
while [[ $# -gt 0 ]]; do
  case "$1" in
    --scale)
      SCALE="$2"
...
# For standalone runs, create a dedicated prepare session folder.
if [[ -z "$SESSION_DIR" ]]; then
  SESSION_DIR="$(benchmark_new_session_dir "$BENCHMARK_ROOT/results" "pgbench-prepare" "$STORAGE_CLASS")"
fi
...

{
  ...
  # Optional destructive path is scoped only to benchmark DB.
  ...
  # Create benchmark DB when missing.
  if [[ "$db_exists" == "0" ]]; then
...
\$\$;"
  ...
  # Initialize benchmark schema/data with requested scale.
  run_pgbench_cmd "$DB_BENCH_DB" -i -s "$SCALE"

  # Save database inventory and sizes for evidence and sanity checks.
