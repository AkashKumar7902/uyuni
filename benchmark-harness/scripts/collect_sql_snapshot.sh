...
# Collect pre/post SQL snapshots and machine-readable extracts for a single run.
# Outputs both human-readable text and JSON/CSV artifacts.
...
usage() {
  cat <<USAGE
Usage: bash benchmark-harness/scripts/collect_sql_snapshot.sh \
  --phase pre|post \
  --run-id <id> \
  --session-dir <path> \
  [--db <dbname>]
...
# Explicit argument validation is important because snapshot file names derive from run-id.
while [[ $# -gt 0 ]]; do
  case "$1" in
    --phase)
      PHASE="$2"
...
# Default to dedicated benchmark DB unless caller overrides.
if [[ -z "$DB_NAME" ]]; then
  DB_NAME="$DB_BENCH_DB"
fi
...
SQL_FILE="$BENCHMARK_ROOT/sql/${PHASE}_snapshot.sql"
[[ -f "$SQL_FILE" ]] || die "Snapshot SQL file not found: $SQL_FILE"

# Text snapshot from canonical SQL file.
# This is the most complete artifact and easiest to inspect during debugging.
run_psql_file "$DB_NAME" "$SQL_FILE" > "$TXT_OUT"

# Additional CSV exports for machine-processing convenience.
# These are intentionally stable subsets used in summaries/comparisons.
...
psql_json_array() {
  # Execute query and wrap rows as JSON array directly in PostgreSQL.
  # This avoids fragile shell JSON assembly.
  local q="$1"
  local db_pod
  db_pod="$(get_db_pod)"
...
# Optional views/extensions are queried only when available.
...
# Final JSON payload references all related artifacts to preserve traceability.
jq -n \
  --arg phase "$PHASE" \
  --arg run_id "$RUN_ID" \
