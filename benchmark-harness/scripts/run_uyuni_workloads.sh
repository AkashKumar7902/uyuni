...
# Run application-level Uyuni workloads and capture benchmark artifacts.
# Each workload run records:
# - timestamps and duration
# - SQL pre/post snapshots
# - Prometheus query-range summaries
# - raw logs and structured JSON run record
...
usage() {
  cat <<USAGE
Usage: bash benchmark-harness/scripts/run_uyuni_workloads.sh --profile smoke|full --session-dir <path>
USAGE
}
...
# Require explicit profile/session from orchestrator to avoid accidental ad-hoc runs.
while [[ $# -gt 0 ]]; do
  case "$1" in
    --profile)
      PROFILE="$2"
...
# Workload run inventory for downstream summarization.
if [[ ! -f "$RUNS_CSV" ]]; then
  cat > "$RUNS_CSV" <<CSV
...
record_run() {
  # Append one normalized run record to session CSV.
  local run_id="$1"
  local workload="$2"
...
run_with_common_artifacts() {
  # Wrapper that applies common benchmark protocol around workload implementation:
  # pre snapshot -> run -> post snapshot -> prometheus collection -> json/csv record.
  local run_id="$1"
  local workload="$2"
  ...
  # Capture DB state before workload execution.
  ...
  # Precise timing window used for Prometheus query_range extraction.
  ...
  local exit_code=0
  # Execute workload-specific function and preserve exit status for evidence.
  ...
  # Capture DB state after workload execution.
  ...
  # Collect Prometheus timeseries for exactly this run interval.
  ...
  # Per-run structured JSON artifact.
  jq -n \
    --arg run_id "$run_id" \
...
repo_sync_channels() {
  # Map configured micro-client repo definitions to channel labels used by spacewalk-repo-sync.
  local mcp
...
run_repo_sync_minimal_impl() {
  # Sequential channel sync to keep run behavior deterministic.
  local run_dir="$1"
  local upod
  upod="$(get_uyuni_pod)"
...
run_metadata_refresh_impl() {
  # Exercises common Uyuni->minion orchestration path with low risk operations.
  local _run_dir="$1"
  local upod
  upod="$(get_uyuni_pod)"
...
run_ui_locust_impl() {
  # Launch disposable locust Job and collect generated CSV statistics.
  local run_dir="$1"
  local users="$2"
  local duration="$3"
...
class UyuniUser(HttpUser):
    ...
    def _extract_csrf_token(self, html: str) -> str | None:
        # Uyuni login page provides CSRF token either as hidden input or JS constant.
        m = re.search(r'name="csrf_token"\s+value="([^"]+)"', html)
        if m:
            return m.group(1)
        m = re.search(r'const\s+csrfToken\s*=\s*"([^"]+)"', html)
        if m:
            return m.group(1)
        return None

    def _extract_bounce_path(self, html: str) -> str:
        # Default bounce is legacy overview page; prefer server-provided value when present.
        m = re.search(r'<div id="init_data_url_bounce"[^>]*>([^<]+)</div>', html)
        if not m:
            return "/rhn/YourRhn.do"
        bounce = m.group(1).strip() or "/rhn/YourRhn.do"
        # Normalize absolute URLs back to path so Locust keeps using configured host.
        if bounce.startswith("http://") or bounce.startswith("https://"):
            parsed = urlparse(bounce)
            bounce = parsed.path or "/rhn/YourRhn.do"
            if parsed.query:
                bounce = f"{bounce}?{parsed.query}"
        if not bounce.startswith("/"):
            bounce = "/" + bounce.lstrip("./")
        return bounce

    def _is_redirected_to_login(self, final_url: str) -> bool:
        return "/rhn/manager/login" in final_url or final_url.rstrip("/").endswith("/rhn/manager")

    def _pick_working_path(self, probe_name: str, candidates: list[str]) -> str:
        # Probe multiple candidate endpoints and keep the first authenticated/working one.
        # Probe requests are marked success even when candidate fails, so they do not pollute
        # benchmark error rate while we discover the right URL for this deployment.
        for path in candidates:
            with self.client.get(path, name=probe_name, verify=False, allow_redirects=True, catch_response=True) as resp:
                if resp.status_code < 400 and not self._is_redirected_to_login(resp.url):
                    resp.success()
                    return path
                resp.success()
        raise RuntimeError(f"No working endpoint found for {probe_name}: {candidates}")

    def on_start(self) -> None:
        # Uyuni login is SPA-based in this lab:
        # GET login page -> extract CSRF -> POST /rhn/manager/api/login -> bounce URL.
        self.client.verify = False
        username = os.getenv("UYUNI_USER", "admin")
        password = os.getenv("UYUNI_PASS", "admin")

        with self.client.get("/rhn/manager/login", name="login_page", allow_redirects=True, catch_response=True) as resp:
            if resp.status_code >= 400:
                resp.failure(f"login page HTTP {resp.status_code}")
                raise RuntimeError(f"Cannot load login page: HTTP {resp.status_code}")
            csrf_token = self._extract_csrf_token(resp.text)
            if not csrf_token:
                resp.failure("missing csrf token on login page")
                raise RuntimeError("Missing CSRF token on login page")
            bounce_path = self._extract_bounce_path(resp.text)
            resp.success()
        ...
        payload = {"login": username.strip(), "password": password.strip()}
        ...
        with self.client.post(
            ...
            if resp.status_code >= 400:
                resp.failure(f"login API HTTP {resp.status_code}")
                raise RuntimeError(f"Login API failed: HTTP {resp.status_code}")
            try:
                body = resp.json()
            except Exception as exc:
                resp.failure(f"login API non-JSON response: {exc}")
                raise RuntimeError("Login API returned non-JSON response") from exc
            if not body.get("success", False):
                resp.failure(f"login API rejected credentials: {body}")
                raise RuntimeError(f"Login rejected credentials: {body}")
            resp.success()

        # Confirm authenticated session by opening bounce page and ensuring no login redirect.
        with self.client.get(bounce_path, name="post_login_bounce", allow_redirects=True, catch_response=True, verify=False) as resp:
            if resp.status_code >= 400 or self._is_redirected_to_login(resp.url):
                resp.failure(f"post-login bounce failed: status={resp.status_code} url={resp.url}")
                raise RuntimeError(f"Post-login bounce failed: status={resp.status_code} url={resp.url}")
            resp.success()

        # Choose working UI endpoints for this deployed version/permissions set.
        self.dashboard_path = self._pick_working_path(
            "dashboard_probe",
            [
                "/rhn/manager/overview",
                "/rhn/manager/dashboard",
                "/rhn/YourRhn.do",
            ],
        )
        self.systems_path = self._pick_working_path(
            "systems_probe",
            [
                "/rhn/manager/systems/list/all",
                "/rhn/manager/systems/overview",
                "/rhn/systems/SystemGroupList.do",
    ...
    def dashboard(self) -> None:
        with self.client.get(self.dashboard_path, name="dashboard_overview", verify=False, allow_redirects=True, catch_response=True) as resp:
            if resp.status_code >= 400 or self._is_redirected_to_login(resp.url):
                resp.failure(f"dashboard request failed: status={resp.status_code} url={resp.url}")
            else:
                resp.success()

    @task(2)
    def systems(self) -> None:
        with self.client.get(self.systems_path, name="systems_list", verify=False, allow_redirects=True, catch_response=True) as resp:
            if resp.status_code >= 400 or self._is_redirected_to_login(resp.url):
                resp.failure(f"systems request failed: status={resp.status_code} url={resp.url}")
            else:
                resp.success()
PY
  ...
  # Job installs locust at runtime to keep manifest lightweight and toolchain-neutral.
        ...
        - name: locust
          ...
          command:
            ...
            - >-
              ...
              -t ${duration}s --host https://${UYUNI_FQDN} --csv=/results/locust --csv-full-history || rc=\$?;
              if [ -f /results/locust_stats.csv ]; then echo "__LOCUST_CSV_BEGIN__locust_stats.csv__"; cat /results/locust_stats.csv; echo "__LOCUST_CSV_END__locust_stats.csv__"; fi;
              if [ -f /results/locust_failures.csv ]; then echo "__LOCUST_CSV_BEGIN__locust_failures.csv__"; cat /results/locust_failures.csv; echo "__LOCUST_CSV_END__locust_failures.csv__"; fi;
              if [ -f /results/locust_exceptions.csv ]; then echo "__LOCUST_CSV_BEGIN__locust_exceptions.csv__"; cat /results/locust_exceptions.csv; echo "__LOCUST_CSV_END__locust_exceptions.csv__"; fi;
              if [ -f /results/locust_stats_history.csv ]; then echo "__LOCUST_CSV_BEGIN__locust_stats_history.csv__"; cat /results/locust_stats_history.csv; echo "__LOCUST_CSV_END__locust_stats_history.csv__"; fi;
              exit "\$rc"
          volumeMounts:
            - name: locustfile
...
YAML
  ...
  failed=0
  while true; do
    # Poll Job status manually to avoid relying on kubectl wait feature variations.
  ...
  local copied_any=0
  # Preferred path: pull files directly from /results while pod is still available.
  if kubectl -n "$BENCH_NAMESPACE" cp "$pod:/results/." "$run_dir/locust-csv" >/dev/null 2>&1; then
    :
  fi

  # Secondary path: direct file streaming via exec (works only while pod container is live).
  local lf
  for lf in locust_stats.csv locust_failures.csv locust_exceptions.csv locust_stats_history.csv; do
    if [[ ! -s "$run_dir/locust-csv/${lf}" ]] && kubectl -n "$BENCH_NAMESPACE" exec "$pod" -- sh -lc "test -f /results/${lf}" >/dev/null 2>&1; then
      kubectl -n "$BENCH_NAMESPACE" exec "$pod" -- sh -lc "cat /results/${lf}" > "$run_dir/locust-csv/${lf}" 2>/dev/null || true
    fi
  done

  # Final fallback path: reconstruct CSVs from explicit markers embedded in pod logs.
  if [[ ! -s "$run_dir/locust-csv/locust_stats.csv" ]]; then
    python3 - "$run_dir/locust.log" "$run_dir/locust-csv" <<'PY'
from __future__ import annotations
...
for line in lines:
    if line.startswith("__LOCUST_CSV_BEGIN__") and line.endswith("__"):
        prefix = "__LOCUST_CSV_BEGIN__"
        name = line[len(prefix):-2].strip()
        if name:
            in_file = out_dir / name
            buffer = []
        continue
    if line.startswith("__LOCUST_CSV_END__") and line.endswith("__"):
        prefix = "__LOCUST_CSV_END__"
        name = line[len(prefix):-2].strip()
        if in_file is not None and in_file.name == name:
            in_file.write_text("\n".join(buffer) + "\n", encoding="utf-8")
        in_file = None
        buffer = []
        continue
...
PY
  ...
  if [[ -s "$run_dir/locust-csv/locust_stats.csv" ]]; then
    # Convert locust aggregate row into concise Markdown summary for proposals/reviews.
...
run_package_download_impl() {
  # Disposable downloader workflow:
  # - discover one baseurl from micro-client repos
  # - fetch/parse repo metadata
  # - pick RPM URLs
  # - download concurrently and summarize
  local run_dir="$1"
  local concurrency="$2"
...
if primary_href.endswith(".gz"):
    ...
    # Prefer python zstandard when available, fallback to zstd CLI for portability.
    try:
...
def fetch_one(url: str) -> dict[str, object]:
    # Download body to count real transferred bytes; avoids relying on Content-Length.
    start = time.time()
...
with summary_md.open("w", encoding="utf-8") as fh:
    # Keep summary intentionally compact; detailed raw data remains in per-run artifacts.
