...
# Run pgbench scenario x client-count matrix and capture full evidence per run.
# Output includes:
# - raw pgbench logs
# - parsed KPI JSON
# - SQL pre/post snapshots
# - Prometheus range summaries
# - aggregated CSV/Markdown summaries
...
usage() {
  cat <<USAGE
Usage: bash benchmark-harness/scripts/run_pgbench_matrix.sh \
...
# Parse optional overrides; defaults are profile-driven below.
while [[ $# -gt 0 ]]; do
  case "$1" in
    --profile)
      PROFILE="$2"
...
# Profile defaults encode the intended smoke/full workload levels.
case "$PROFILE" in
  smoke)
...
# Create run inventory CSV once per session.
if [[ ! -f "$RUNS_CSV" ]]; then
  cat > "$RUNS_CSV" <<CSV
...
parse_pgbench_output() {
  ...
  # Parse common pgbench output fields in a robust, regex-based manner.
  # Missing fields are emitted as null/empty values rather than hard failure.
...
for scenario in $SCENARIOS; do
  # Iterate each client count within current scenario.
  while IFS= read -r client; do
    [[ -n "$client" ]] || continue

    if [[ "$PGBENCH_WARMUP" == "1" ]]; then
      # Warmup is optional and intentionally excluded from run summaries.
      ...
      case "$scenario" in
        builtin_default)
          ...
          ;;
        readonly_custom)
          run_pgbench_cmd "$DB_BENCH_DB" -c "$client" -j "$client" -T "$DURATION" -P 10 -f "$BENCHMARK_ROOT/sql/pgbench_readonly.sql" > "$warmup_raw" 2>&1 || true
          ;;
        wal_small_update)
          run_pgbench_cmd "$DB_BENCH_DB" -c "$client" -j "$client" -T "$DURATION" -P 10 -f "$BENCHMARK_ROOT/sql/pgbench_wal_small_update.sql" > "$warmup_raw" 2>&1 || true
          ;;
      esac
    fi

    for run_n in $(seq 1 "$RUN_COUNT"); do
      # Per-run directory keeps raw logs and parsed artifacts isolated and deterministic.
      ...
      # Snapshot DB state before workload.
      ...
      # Record precise workload window for Prometheus query_range calls.
      ...
      case "$scenario" in
        builtin_default)
          ...
          ;;
        readonly_custom)
          run_pgbench_cmd "$DB_BENCH_DB" -c "$client" -j "$client" -T "$DURATION" -P 10 -f "$BENCHMARK_ROOT/sql/pgbench_readonly.sql" > "$raw_log" 2>&1 || exit_code=$?
          ;;
        wal_small_update)
          run_pgbench_cmd "$DB_BENCH_DB" -c "$client" -j "$client" -T "$DURATION" -P 10 -f "$BENCHMARK_ROOT/sql/pgbench_wal_small_update.sql" > "$raw_log" 2>&1 || exit_code=$?
          ;;
        *)
      ...
      # Snapshot DB state after workload.
      ...
      # Pull Prometheus metrics exactly for this run window.
      ...
      # Normalize raw output into structured fields for downstream summaries.
      ...
      # Write per-run JSON record for auditability and machine processing.
      jq -n \
        --arg run_id "$run_id" \
        --arg profile "$PROFILE" \
      ...
      # Append one row to global run inventory CSV.
...
# Build scenario/client aggregate summary after all runs complete.
