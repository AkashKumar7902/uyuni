...
"""Build top-level benchmark session summaries from run inventories.
...
def read_csv(path: Path) -> list[dict[str, str]]:
    """Read CSV into list of dict rows; return empty list when file is missing."""
    if not path.exists():
        return []
...
def safe_float(value: str | None) -> float | None:
    """Best-effort conversion used for optional numeric cells."""
...
def parse_session_env(path: Path) -> dict[str, str]:
    """Parse simple key=value metadata file saved at session start."""
    out: dict[str, str] = {}
    if not path.exists():
        return out
...
def summarize_pgbench(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    """Aggregate pgbench KPI rows by (scenario, client-count)."""
...
def summarize_uyuni(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    """Aggregate workload durations by workload name."""
...
def summarize_prom(rows: list[dict[str, str]]) -> tuple[list[dict[str, str]], list[str]]:
    """Aggregate Prometheus query summaries and track queries with missing data."""
...
def write_csv(path: Path, rows: list[dict[str, str]]) -> None:
    """Write CSV file using dict keys as stable header order."""
...
def main() -> None:
    # Load session inputs produced by benchmark scripts.
    parser = argparse.ArgumentParser(description="Summarize benchmark session")
    parser.add_argument("session_dir", help="Path to one benchmark session directory")
    ...
    # Compute aggregate summaries used by both CSV and Markdown outputs.
    ...
    benchmark_summary_md = session_dir / "benchmark-summary.md"
    ...
    # Report unavailable DB views/extensions for context when metrics appear incomplete.
    ...
    # Consolidate run errors across workload inventories.
    errors = [r for r in pgbench_rows + uyuni_rows if r.get("exit_code") not in {"0", ""}]

    with benchmark_summary_md.open("w", encoding="utf-8") as fh:
    ...
    print(str(benchmark_summary_md))
